    ...
    <form @submit.prevent="submitForm">
      <div class="space-y-12">
        <div class="border-b border-gray-900/10 pb-12">
          <h2 class="block uppercase tracking-wide text-gray-700 text-base top-0 font-bold mb-2">
            Register To Play
          </h2>
            ...
            <div class="sm:col-span-3">
              ...
              <div class="mt-2">
                <input
                  ...
                  :class="{ 'border-red-500': !isNickNameValid && isNickNameTouched }"
                  @blur="isNickNameTouched = true"
                  class="appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white"
                />
...
const isNameValid = computed(() => {
  return /^[a-zA-Z' ']+$/.test(formData.value.name)
})

const isNickNameTouched = ref(false)

const isNickNameValid = computed(() => {
  return /^[a-zA-Z]+$/.test(formData.value.nickName)
})
...
const isDateOfBirthValid = computed(() => {
  return formData.value.dateOfBirth != '' && formData.value.dateOfBirth <= getTodayDate()
})
...
async function submitForm() {
  try {
    ...
    if (error instanceof Error) {
      ...
      if (errorMessage === '32 members have already registered') {
